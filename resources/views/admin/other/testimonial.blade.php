...
@section('breadcrumb')
<h1 class="d-flex flex-column text-dark fw-bold fs-3 mb-0">Testimonials</h1>
...
<div class="card">
    <div class="card-body p-0">
        ...
        <div class="card-px text-center py-20 ">
            <h2 class="fs-2x fw-bold mb-10">No Testimonials Found</h2>
            <p class="text-gray-400 fs-4 fw-semibold mb-10">Looks like you do not have any testimonials here.
                <br />If you want to add a testimonials, click on the button below.
            </p>
            </p>
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" data-kt-customer-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Name" />
            </div>
        </div>
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="data_table">
            <thead>
                <tr class="text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                    ...
                    <th class="text-center min-w-50px">company Name</th>
            ...
            <tbody class="fw-semibold text-gray-600">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <button class="btn btn-icon btn-active-light-primary w-30px h-30px me-3" onclick="getTestimonialData('{{$data->id}}')" data-bs-toggle="modal" data-bs-target="#updatemodal{{$data->id}}">
                            <span class="svg-icon svg-icon-3">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="modal-dialog modal-dialog-centered mw-1000px">
                        <div class="modal-content rounded">
                            <div class="modal-header">
                                <h1 class="modal-tital w-100 text-center"> Update Testimonial of {{ $data->name }} </h1>
                            ...
                            <form autocomplete="off" action="{{url('testimonial/update')}}" enctype="multipart/form-data" method="post" id="kt_modal_add_form{{$data->id}}">
                                <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                                    ...
                                    <div class="row g-9 mt-3">
                                        <div class="col-md-6 fv-row">
                                            ...
                                            <input type="text" class="txtOnly form-control form-control-solid" placeholder="Enter Name" name="name" id="name{{$data->id}}" value="{{$data->name}}">
                                        </div>
                                        <div class="col-md-6 fv-row">
                                            <label class="required fs-6 fw-semibold mb-2">Company Name</label>
                                            <input type="text" class="form-control form-control-solid" placeholder="Enter Company Name" name="companyName" id="companyName{{$data->id}}" value="{{$data->companyName}}">
                                        </div>
                                        <div class="col-md-3 fv-row">
                                            <label class="required fs-6 fw-semibold mb-2">Sequence</label>
                                            <input type="number" class="form-control form-control-solid" placeholder="Enter Sequence" name="sequence" id="sequence{{$data->id}}" value="{{$data->sequence}}" min="0">
                                        </div>
                                        <div class="col-md-3 fv-row">
                                        ...
                                        <div class="col-md-6 fv-row">
                                            ...
                                            <textarea class="form-control form-control-solid" placeholder="Enter Comment" name="comment" id="comment{{$data->id}}" rows="5" onkeyup="updatevalidchar('{{$data->id}}')">{{$data->comment}}</textarea>
                                            <span id="ValidationMessage{{$data->id}}"></span>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="modal fade" id="deleteModal{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
                    <div class="modal-dialog ">
                        <div class="modal-content">
                            ...
                            <form action="{{url('testimonial/delete')}}" id="UserDeleteForm{{$data->id}}" method="post">
                                @csrf
                                <input type="hidden" value="{{$data->id}}" name="testimonialId">
                                <div class="modal-body">
                                    <span>Are you sure you want to delete testimonial of {{$data->name}}? <br> Action cannot be reverted</span>
                                </div>
                                <div class="modal-footer">
...
<div class="modal fade" id="addmodal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered mw-1000px">
        <div class="modal-content rounded">
            <div class="modal-header">
                <h1 class="modal-tital w-100 text-center"> Add Testimonial </h1>
                <div class="btn btn-sm btn-icon btn-active-color-primary" data-bs-dismiss="modal">
                    <span class="svg-icon svg-icon-1">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect opacity="0.5" x="6" y="17.3137" width="16" height="2" rx="1" transform="rotate(-45 6 17.3137)" fill="currentColor" />
            ...
            <form autocomplete="off" action="{{url('/testimonial/add')}}" enctype="multipart/form-data" method="post" id="kt_modal_add_form">
                <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                    @csrf
                    <div class="row g-9 mt-3">
                        <div class="col-md-6 fv-row">
                        ...
                        <div class="col-md-6 fv-row">
                            <label class="required fs-6 fw-semibold mb-2">Company Name</label>
                            <input type="text" class="form-control form-control-solid" placeholder="Enter Company Name" name="companyName" id="companyName">
                        </div>
                        <div class="col-md-3 fv-row">
                            <label class="required fs-6 fw-semibold mb-2">Sequence</label>
                            <input type="number" class="form-control form-control-solid" placeholder="Enter Sequence" name="sequence" id="sequence" min="0">
                        </div>
                        <div class="col-md-3 fv-row">
                            <label class="fs-6 fw-semibold mb-2">Status</label>
                        ...
                        <div class="col-md-6 fv-row">
                            <label class="required fs-6 fw-semibold mb-2">Comment</label>
                            <textarea class="form-control form-control-solid" placeholder="Enter Comment" name="comment" id="comment" rows="5" onkeyup="validatechar()"></textarea>
...
@section('scripts')
<!--character accept only validation-->
<script>
                    ...
                    n = FormValidation.formValidation(r, {
                        fields: {
                            ...
                            companyName: {
                                validators: {
                                    notEmpty: {
                                        message: "Enter company name is required"
                                    },
                                }
                            },
                            sequence: {
                                validators: {
                                    notEmpty: {
                                        message: "Enter sequence number is required"
                                    },
                                }
                            },
                            comment: {
...
<script>
    ...
    function updatevalidchar(id) {
        var comment = document.getElementById('comment' + id).value;
        var ValidationMessage = document.getElementById('ValidationMessage' + id);
